...
@val
external querySelector: string => option<Dom.element> = "window.document.querySelector" /* wut */
...
let updateList = (): unit => {
  ...
  Model.getNotes() |> List.iter(note => {
    let li = document |> Document.createElement("li")
    let text = if String.length(note.text) > 10 {
...
let createList = () => {
  ...
  button |> Element.addEventListener("click", _e => {
...
let onInput = _event => {
  ...
  | Some(id) =>
    let _ = Model.updateNote({text: text, id: id})
    Model.saveNotes()
  }
  updateList()
  ()
...
let init = (mainSelector: string): unit => {
  Model.loadNotes()
  let mainEl = switch document |> Document.querySelector(mainSelector) {
